...
public class BrewLore {
    ...
    public void updateDistillLore(boolean qualityColor) {
        // El Reino 6: NO mostrar la linea de lore de destilado (lleva un marcador invisible + texto
        // que rompe el match por nombre+lore de las entregas a NPC de PikaMug). El nº de destilaciones
        // sigue en el NBT del brew; solo se oculta la LINEA de lore. Asi cualquier cerveza se entrega
        // solo por su NOMBRE y la entrega NUNCA falla.
        if (brew != null) return;
    ...
    public void updateAgeLore(boolean qualityColor) {
        // El Reino 6: NO mostrar la linea de "X Años / Añejado en tonel". Ese numero VARIA con el
        // tiempo real de barrica -> hacia imposible casar la cerveza en una entrega a NPC (PikaMug).
        // La edad sigue en el NBT (afecta a la calidad); solo se oculta la LINEA de lore.
        if (brew != null) return;
